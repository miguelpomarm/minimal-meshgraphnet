...
│   ├── __init__.py
│   ├── run_model.py               # CLI entry point (train / eval)
...
├── download_dataset.sh            # Download the original DeepMind dataset
├── requirements.txt
├── LICENSE
└── README.md
```
...
Download the CylinderFlow dataset from DeepMind's public storage:
 
```bash
bash download_dataset.sh cylinder_flow /tmp/datasets
```
 
This downloads `meta.json`, `train.tfrecord`, `valid.tfrecord` and `test.tfrecord` into `/tmp/datasets/cylinder_flow/`. Upload the dataset to your own S3 bucket before running the next step.
